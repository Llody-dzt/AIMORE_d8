...
{	
	#if UART_CHANGE
		return uart_dev_putchar(st);
	#else
		return uart2_dev_putchar(st);
	#endif	
}
...
void uart2_dev_send_byte(unsigned char data)
{
	while (!uart2_tx_fifo_empty_getf());
...
{
	#if UART_CHANGE
		uart_dev_send_byte(data);
	#else
		uart2_dev_send_byte(data);
	#endif
}
