...
#if (HILINK)            
    {
        extern char *hi_get_dev_name(void);
        if(strlen(hi_get_dev_name())){
            app_env.dev_name_len = strlen(hi_get_dev_name());
            // Get default Device Name (No name if not enough space)
            memcpy(&app_env.dev_name[0], hi_get_dev_name(), app_env.dev_name_len);
        }
        //uart_printf(".........name %s", hi_get_dev_name());
    }						
#else
            ...
            switch(cmd->info.host.mode)
            {
                ...
                default: break; // do nothing
						  ...
						  cmd->info.host.adv_data_len=3;					            						
							memcpy(&cmd->info.host.adv_data[cmd->info.host.adv_data_len],APP_FTMP_ADV_SERVICE_UUID, APP_FTMP_ADV_SERVICE_UUID_LEN);			
							cmd->info.host.adv_data_len += APP_FTMP_ADV_SERVICE_UUID_LEN;
						
//							cmd->info.host.adv_data_len=7;
//							memcpy(&cmd->info.host.adv_data[cmd->info.host.adv_data_len],APP_FTMP_ADV_DATA_UUID, APP_FTMP_ADV_DATA_UUID_LEN);	
//							cmd->info.host.adv_data_len += APP_FTMP_ADV_DATA_UUID_LEN;	
						#else
							cmd->info.host.adv_data_len=3;					
							memcpy(&cmd->info.host.adv_data[cmd->info.host.adv_data_len],APP_FFF0_ADV_DATA_UUID, APP_FFF0_ADV_DATA_UUID_LEN);
        ...
        if (device_name_avail_space > 2)
        {
...
#if (HILINK)            
            {
                ...
                if(strlen(hi_get_dev_name())){								
									#if 1
										device_name_length = strlen("Yijian-D8-0001");
										memcpy(&device_name_temp_buf[0], "Yijian-D8-0001", strlen("Yijian-D8-0001"));
									#else
                    device_name_length = strlen(hi_get_dev_name());
                    // Get default Device Name (No name if not enough space)
                    memcpy(&device_name_temp_buf[0], hi_get_dev_name(), device_name_length);
									#endif
               
								}
							//	uart_printf(".........name1 %s", hi_get_dev_name());
            }
#else
            {
                ...
                cmd->info.host.adv_data_len += (device_name_length + 2);					
					...
					/*if(APP_ADV_MANU_SPECIFIC_DATA_LEN>0)
					{
						memcpy(&cmd->info.host.adv_data[cmd->info.host.adv_data_len],
									 APP_ADV_MANU_SPECIFIC_DATA,APP_ADV_MANU_SPECIFIC_DATA_LEN);
						cmd->info.host.adv_data_len += APP_ADV_MANU_SPECIFIC_DATA_LEN;
            }*/
...
#endif                    
        {
						...
						memcpy(&cmd->info.host.scan_rsp_data[cmd->info.host.scan_rsp_data_len], APP_FTMP_ADV_SERDATA_UUID, APP_FTMP_ADV_SERDATA_UUID_LEN);
					  cmd->info.host.scan_rsp_data_len += APP_FTMP_ADV_SERDATA_UUID_LEN;
					
						cmd->info.host.scan_rsp_data_len=9;
						memcpy(&cmd->info.host.scan_rsp_data[cmd->info.host.scan_rsp_data_len],APP_FTMP_ADV_DATA_UUID, APP_FTMP_ADV_DATA_UUID_LEN);	
						cmd->info.host.scan_rsp_data_len += APP_FTMP_ADV_DATA_UUID_LEN;	
					
            cmd->info.host.scan_rsp_data_len = 16;
						//memcpy(&cmd->info.host.scan_rsp_data[cmd->info.host.scan_rsp_data_len], APP_SCNRSP_DATA, APP_SCNRSP_DATA_LEN);
