...
void app_sec_init()
{
	...
	flash_read(FLASH_SPACE_TYPE_NVR, APP_SEC_MSG_ADD, sizeof(app_nvr_tag), (uint8_t*)&app_nvr_tag);
...
void app_sec_remove_bond(void)
{
	...
	if (app_sec_env.bonded == true)
	{
		...
		app_sec_env.bonded = false;
		flash_read(FLASH_SPACE_TYPE_NVR, APP_SEC_MSG_ADD, sizeof(app_nvr_tag), (uint8_t*)&app_nvr_tag);
		flash_erase_sector(FLASH_SPACE_TYPE_NVR, APP_SEC_MSG_ADD);
		app_nvr_tag.bond = 0x0;
		memset(&app_nvr_tag.ltk, 0x0, sizeof(struct gapc_ltk));
		flash_write(FLASH_SPACE_TYPE_NVR, APP_SEC_MSG_ADD, sizeof(app_nvr_tag), (uint8_t*)&app_nvr_tag);
	}
	app_sec_env.bonded = false;
	app_sec_env.peer_pairing = false;
	app_sec_env.peer_encrypt = false;
