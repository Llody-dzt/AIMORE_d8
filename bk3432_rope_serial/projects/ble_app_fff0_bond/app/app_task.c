		...
		if (!appm_add_svc())
		{
			...
			appm_start_advertising();
                ...
                ke_timer_set(APP_PERIOD_TIMER, TASK_APP, 10);
							ke_timer_set(APP_GPIO_SCAN_TIMER, TASK_APP, 10);
                #endif
		}
	}
	break;
...
void gpio_wkup_init(void)
{
	//appm_stop_advertising();
	ke_timer_clear(APP_GPIO_SCAN_TIMER, TASK_APP);
	//gpio_config(0X33,INPUT, PULL_HIGH);
	REG_APB5_GPIO_WUATOD_TYPE=0; //初始化
	REG_APB5_GPIO_WUATOD_ENABLE = 0; //初始化
	REG_APB5_GPIO_WUATOD_STAT = 0xFF; //初始化
	
	REG_APB5_GPIO_WUATOD_TYPE |= 1<<3; //下降沿   P33
	REG_APB5_GPIO_WUATOD_ENABLE |= 1<<3;// 使能
}
void power_off(void)
{
	UART_PRINTF("power_off\r\n");
	appm_disconnect();
	appm_stop_advertising();
...
static int app_gpio_scan_timer_handler(ke_msg_id_t const msgid,
                                          void *param,
                                          ke_task_id_t const dest_id,
                                          ke_task_id_t const src_id)
{
#if 0
		if(0 == gpio_get_input(0x33))
    {
        if(ke_state_get(TASK_APP) == APPM_ADVERTISING)
        {
            
						UART_PRINTF("advertising disenable\r\n");
        }
    }
    else
    {
...
const struct ke_msg_handler appm_default_state[] =
{
  ...
  {APP_GATTC_EXC_MTU_CMD,		    (ke_msg_func_t)gattc_mtu_exchange_req_handler},
	{APP_GPIO_SCAN_TIMER,				(ke_msg_func_t)app_gpio_scan_timer_handler},
};
